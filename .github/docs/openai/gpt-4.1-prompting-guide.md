...
1. **Markdown** – Start here, using markdown headings for major sections and subsections (including deeper hierarchy, down to `####`). Use inline back-ticks or fenced blocks to wrap code precisely, and standard numbered or bulleted lists as needed.  
2. **XML** – Also performs well. XML lets you wrap a section with clear start/end tags, add metadata on the tags, and nest content. For example:
...
3. **JSON** – Highly structured and well-understood by the model in coding contexts, but it can be verbose and require character escaping that adds overhead.

**Guidance for adding a large number of documents/files to the input context**

- **XML** performed well in long-context testing.  
  - Example: `<doc id='1' title='The Fox'>The quick brown fox jumps over the lazy dog</doc>`  
- **Lee et al. format** ([paper PDF](https://arxiv.org/pdf/2406.13121)) also performed well:  
...
The model is trained to understand a variety of structures. Use your judgment to choose a format that is **visually clear** and “stands out” to the model. (E.g., if your source docs already contain lots of XML, using XML as the delimiter will be less effective.)

## Caveats

- In isolated cases the model resists producing **very long, repetitive outputs** (e.g., analyzing hundreds of items one-by-one). If you need that, strongly instruct the model to output everything in full, or break down the problem into smaller chunks.  
- Rarely, **parallel tool calls** can be incorrect. If you see issues, test with `parallel_tool_calls=false`.

---

# Appendix: Generating and Applying File Diffs

Developers tell us that accurate, well-formed diff generation is critical for coding tasks. The GPT-4.1 family features substantially improved diff capabilities relative to previous GPT models. While GPT-4.1 can generate *any* diff format given clear instructions and examples, we open-source below one **recommended diff format** that the model was extensively trained on.
...
```python
APPLY_PATCH_TOOL_DESC = """This is a custom utility that makes it more convenient
to add, remove, move, or edit code files. `apply_patch` effectively allows you to
execute a diff/patch against a file, but the format of the diff specification is
unique to this task, so pay careful attention to these instructions.

To use the `apply_patch` command, pass a message of the following structure as
"input":
...
Where [YOUR_PATCH] is the actual content of your patch, specified in the V4A
diff format:

*** [ACTION] File: [path/to/file]       ← ACTION is Add | Update | Delete

For each snippet of code to be changed:
[context_before]          ← See below
- [old_code]              ← Precede old code with a minus
+ [new_code]              ← Precede replacement code with a plus
[context_after]

Context rules
-------------
* Show 3 lines above and 3 lines below each change by default.
* If two changes are within 3 lines, don’t duplicate context.
* If 3 lines isn’t enough to locate the snippet uniquely, add an @@ hint:
...
* If needed, chain multiple @@ hints:
...
*(Code continues … full implementation retained in source)*

## Other Effective Diff Formats

If you want to experiment, we found two additional formats with high success rates:
...
1. **Avoid line numbers** (they rely on context matching instead).  
2. Provide *both* the exact code to replace **and** the replacement code with clear delimiters.

Use whichever format best suits your tooling and team preferences.
